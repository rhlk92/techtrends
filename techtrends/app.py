...
def post(post_id):
    ...
    if post is None:
        app.logger.error('A non-existing article is accessed'.format(post_id))
...
def get_metrics():
    ...
    post_count = article_count[0]

    return db_connection_count, post_count


@app.route('/metrics')
def metrics():
    db_connection_count, post_count = get_metrics()
